...
The existing `pytest_runtest_makereport` hook in `conftest.py` (previously failure-screenshot-only) is extended — not duplicated, and with no early return — to call `browserstack_status_payload()` and `page.evaluate("_ => {}", f"browserstack_executor: {json.dumps(payload)}")` when `report.when == "call"` and `CLOUD_GRID_PROVIDER=browserstack`. The magic string is passed as the function *argument*, not as the JS expression itself — BrowserStack's CDP proxy watches the argument value for the `browserstack_executor:` prefix; passing it as the expression directly would not be valid JS and would silently fail inside the `try/except`. The status uses `report.passed` rather than `not report.failed`, so a test that calls `pytest.skip()` mid-body is not mislabeled as `passed` on the dashboard. The reason text contains only the pytest node ID — never exception text, traceback, or test data — consistent with this repo's existing redaction discipline. The call is wrapped in `try/except Exception: pass`; a marking failure never affects the test outcome, the failure screenshot/HTML capture logic below it, or any required signal.
