...
| `Cloud Grid` | Provider-aware cloud-grid smoke execution, preflight-gated, 3-browser matrix (chromium, firefox, webkit) — advisory only; Sauce Labs active, BrowserStack readiness-only | `schedule` and `workflow_dispatch` only; never on PR or push | `Docker Test Suite` |
