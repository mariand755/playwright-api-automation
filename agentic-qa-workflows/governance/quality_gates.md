...
| `Cloud Grid` | Sauce Labs cloud-grid smoke execution, preflight-gated, chromium — advisory only | `schedule` and `workflow_dispatch` only; never on PR or push | `Docker Test Suite` |
...
`Cloud Grid` is advisory — `continue-on-error: true`; runs on nightly and `workflow_dispatch` only; not listed in branch protection required checks. Cloud-grid execution is gated on `READY` preflight status from `scripts/cloud_grid_preflight.py` — all `SKIPPED_*` statuses skip cloud execution and exit 0. Cloud Grid failures surface as signal but do not block PRs or merges.
