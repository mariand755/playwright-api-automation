...
This script is stdlib-only and contains no repo-specific logic. Three spots need
updating when applying this to a new project:

1. ARTIFACT_JSON (below): path to the release-readiness JSON produced by release_gate.py.
   Default matches the reference implementation artifact layout. Change if your CI writes
   the artifact to a different path.

2. job_labels in build_message_lines(): update the display names to match your CI job names.
   The env var keys (docker_test_suite, api_tests, ui_tests) correspond to the env vars
   DOCKER_TEST_SUITE_RESULT, API_TESTS_RESULT, UI_TESTS_RESULT set in your notify job —
   rename both the keys here and the env var names in your ci.yml notify step together.

3. "Release Gate" label in build_message_lines(): the label reads "Release Gate: ..." in
   notifications. Adjust the label text if your project uses a different gate name.

See blueprint/README.md Section 3 (Notification Policy Pattern) for full adaptation guide.
"""
...
# Adaptation point 1: update if your CI writes the artifact to a different path.
...
def build_message_lines(
    ...
    # Adaptation point 2: update display names to match your CI job names.
    # Keys must match the env var names set in your notify job (e.g. DOCKER_TEST_SUITE_RESULT).
    job_labels = [
    ...
    # Adaptation point 3: rename this label if your release gate has a different name.
    if data is None:
        lines.append(
        ...
        lines.append(
            "Release Gate: ⚠️ Skipped — smoke-only run does not produce a release gate decision"
        )
    else:
        gate_str = str(data.get("overall_decision", "UNKNOWN"))
        gate_emoji = "✅" if gate_str == "GO" else "❌"
        gate_line = f"Release Gate: {gate_emoji} {gate_str}"
